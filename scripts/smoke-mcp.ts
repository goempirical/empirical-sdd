...
try {
  ...
  for (const args of [["version"], ["--version"], ["-v"]]) {
    ...
    if (version.stdout !== "0.25.0\n" || packageJson.version !== "0.25.0") {
  ...
  for (const args of [[], ["help"], ["--help"], ["-h"]]) {
    ...
    if (
      !help.stdout.includes("empirical v0.25.0")
  ...
  console.log("Bundled 0.25 OAuth-first Init, tracker, uninstall, receipt, routing, YOLO, worktree, CLI, and MCP smoke passed.");
} finally {
  await client.close();
